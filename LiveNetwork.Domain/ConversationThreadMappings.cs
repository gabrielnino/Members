            ...
            foreach (var t in threads)
            {
                ...
                if (t.Communications is not null)
                {
                    foreach (var xc in t.Communications)
                    {
                        ...
                        //{
                        //    var status = ParseEnumSafe<InviteStatus>(xc.Status, InviteStatus.Draft);
                        //    var invite = new Invite(commId, xc.Content ?? string.Empty, xc.Experiment ?? string.Empty, status);
                        //    dp.AddInvite(invite);
                        //}
                        //else if (string.Equals(xc.TypeName, nameof(Message), StringComparison.Ordinal))
                        //{
                        //    var status = ParseEnumSafe<MessageStatus>(xc.Status, MessageStatus.Draft);
                        //    var message = new Message(commId, xc.Content ?? string.Empty, xc.Experiment ?? string.Empty, status);
